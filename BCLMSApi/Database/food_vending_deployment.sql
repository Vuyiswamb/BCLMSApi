SET XACT_ABORT ON;
BEGIN TRANSACTION;
IF COL_LENGTH('dbo.Applications','FoodVendingDetailsJson') IS NULL ALTER TABLE dbo.Applications ADD FoodVendingDetailsJson NVARCHAR(MAX) NULL;
GO
...
BEGIN
    ...
    SELECT
        ...
        applications.FoodVendingDetailsJson,
...
CREATE OR ALTER PROCEDURE dbo.usp_Applications_Insert
    ...
    @FoodVendingDetailsJson NVARCHAR(MAX) = NULL,
...
CREATE OR ALTER PROCEDURE dbo.usp_Applications_ResubmitRejectedForCustomer
    ...
    @TradeStandBusinessType NVARCHAR(120) = NULL,
    @FoodVendingDetailsJson NVARCHAR(MAX) = NULL,
    @AreaCategory NVARCHAR(100) = NULL,
    @EventStartDate DATE = NULL,
    @EventEndDate DATE = NULL,
    @Latitude DECIMAL(9,6) = NULL,
    @Longitude DECIMAL(9,6) = NULL,
...
BEGIN
    ...
    SET EmailAddress = @EmailAddress,
        ...
        FoodVendingDetailsJson = @FoodVendingDetailsJson,
        AreaCategory = COALESCE(@AreaCategory, AreaCategory),
    ...
    UPDATE steps SET AssignedGroupId = groups.GroupId
    FROM dbo.ApplicationWorkflowSteps steps
    JOIN dbo.Applications a ON a.ApplicationId = steps.ApplicationId
    JOIN dbo.Groups groups ON groups.GroupName = CASE WHEN @AreaCategory = 'Restricted' THEN 'Compliance Officer' ELSE 'Metro Police' END
    WHERE a.ApplicationId = @ApplicationId AND a.LicenceType LIKE '%Food Vending%' AND steps.StepName LIKE '%Inspection%';
    SELECT
        ApplicationId,
        TrackingNumber,
        ApplicantName,
...
GO
IF NOT EXISTS (SELECT 1 FROM dbo.AttachmentTypes WHERE TypeName='Site Approval') INSERT dbo.AttachmentTypes(TypeName,Description) VALUES('Site Approval','Site approval for food vending');
IF NOT EXISTS (SELECT 1 FROM dbo.AttachmentTypes WHERE TypeName='Food handling Certificate') INSERT dbo.AttachmentTypes(TypeName,Description) VALUES('Food handling Certificate','Required when breakfast items are selected');
COMMIT TRANSACTION;
GO
